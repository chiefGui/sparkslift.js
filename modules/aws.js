...
AWS.prototype.getRequestUrl = function () {
    ...
    if (Object.keys(this.params).length > 0) u =+ '?' + this.getCanonicalQueryString();
    return u;
};

AWS.prototype.getCanonicalHeaders = function () {
    var h = this.headers;
    return Object.keys(h).sort().map( function (name) {
...
AWS.prototype.getCanonicalQueryString = function () {
    var p = this.params;
    return Object.keys(p).sort().map( function (key) {
...
AWS.prototype.getAuthHeader = function () {
    return 'AWS4-HMAC-SHA256 Credential=' +
        this.key + '/' + this.getAwsCredentialScope() +
        ', SignedHeaders=' + this.getAwsSignedHeaders() +
        ', Signature=' + this.getAwsCredentialSignature();
};

AWS.prototype.getAwsCredentialSignature = function () {
    return hmac(this.getAwsCredentialSignatureKey(), "HEX", this.getAwsCredentialSignatureValue());
}
...
AWS.prototype.getAwsSignedHeaders = function () {
    return Object.keys(this.headers).sort().map( function (name) {
        return name.toLowerCase().trim();
    }).join(';');
};

AWS.prototype.getAwsCredentialScope = function () {
    return [this.awsDateTimeFormatShort(), this.region, this.service, 'aws4_request'].join('/');
};
